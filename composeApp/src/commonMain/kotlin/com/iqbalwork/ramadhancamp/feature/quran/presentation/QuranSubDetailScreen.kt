...
@Composable
fun QuranSubDetailScreen() {
    val viewModel: QuranViewModel = rememberViewModel()

    Column(
        modifier = Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(16.dp),
        verticalArrangement = Arrangement.spacedBy(8.dp),
    ) {
        Text("Quran SubDetail", style = MaterialTheme.typography.headlineMedium)
        DemoSection("Back to Root") {
            DemoButton("Back to Quran Main")             { viewModel.backToMain() }
            DemoButton("Back to Quran Main with Result") { viewModel.backToMainWithResult() }
        }
        DemoSection("Back") {
